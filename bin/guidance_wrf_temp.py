...
def readCoefficient(station, d):
    if os.path.isdir(d + '/wrf_temp/{}'.format(station)) is False:
        #[[1], [f['temp']], [f['ew']], [f['ww']], [f['nw']], [f['sw']], [f['cldlo']], [f['cldmi']], [1. - f['cldlo']], [1. - f['cldmi']], [f['prec']]])
...
def makeGuidance(db, init, coeff):
    ...
    with sqlite3.connect(db) as con:
        ...
        for station in stations:
            ...
            sql = ''' SELECT ft, t2, u10, v10, cld_lo, cld_mo, cld_hi, rain_rc + rain_rnc AS prec FROM WRFFcst WHERE Station = ? AND init = ?;'''
            ...
            for r in rows:
                ...
                C = np.mat([[1], [r['t2']], [ew], [ww], [nw], [sw], [r['cld_lo']], [r['cld_mo']], [1. - r['cld_lo']], [1. - r['cld_mo']], [r['prec']]])
                v = int(round(float(C.T * x_hat[date.hour]), 1) * 10)
                sql2 = ''' UPDATE StationFcstHourly SET Temp = ? WHERE Station = ? AND init = ? AND ft = ?;'''
                cur.execute(sql2, (v, station, init, int(r['ft'])))
                
                print(date, station, float(C.T * x_hat[date.hour]), r['t2'], v)
            con.commit()
    return
